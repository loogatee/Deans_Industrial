...
//  The data is destined for USART1.
...
//
void UD_Send( u32 otype, char *sptr, u32 *completionptr, u32 aval )
{
...
{
    UD_Send( SERO_TYPE_ONECHAR, 0, 0, (u32)ch );
}

void UD_PrintSTR( const char *pstr )
{
    UD_Send( SERO_TYPE_STR, (char *)pstr, 0, 0 );
}

void UD_Print32( const char *pstr, u32 val )
{
    UD_Send( SERO_TYPE_32, (char *)pstr, 0, val );
}

void UD_Print32N( const char *pstr, u32 val )
{
    UD_Send( SERO_TYPE_32N, (char *)pstr, 0, val );
}

void UD_Print8( const char *pstr, u8 val )
{
    UD_Send( SERO_TYPE_8, (char *)pstr, 0, (u32)val );
}

void UD_Print8N( const char *pstr, u8 val )
{
    UD_Send( SERO_TYPE_8N, (char *)pstr, 0, (u32)val );
}

void UD_Print16( const char *pstr, u16 val )
{
    UD_Send( SERO_TYPE_16, (char *)pstr, 0, (u32)val );
}

void UD_Print16N( const char *pstr, u16 val )
{
    UD_Send( SERO_TYPE_16N, (char *)pstr, 0, (u32)val );
}
...
int _write( void *fp, char *buf, u32 len )
{
    ...
    UD_PrintSTR( (const char *)&serd_pfbuf[serd_pfindex] );
