...
extern void daemon(int nochdir, int noclose);
...
static void *worker_run(void *a) {
	...
	for (;;) {
		...
		if (script) {
			...
			if (split) {
				*split = '\0';
				if( (rc = chdir(script)) < 0 ) { logit("chdir problem worker"); }
				*split = SEP;
			}
		}
...
int main(int arc, char** argv) {
	...
	if (conf->logfile)
	{
		if( (jfd = freopen(conf->logfile, "w", stderr)) == NULL )
			logit("freopen call failed");
	}
	...
	tv.tv_nsec = (conf->sweep % 1000) * 100000;*/
	for (;;) {
